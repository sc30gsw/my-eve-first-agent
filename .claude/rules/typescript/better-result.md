---
description: better-result best practices for typed errors, composition, and eve boundaries
...
- Wrap throwing third-party calls (connection/MCP clients, fetch, SDK calls inside a tool's `execute`) at the boundary with `Result.try` or `Result.tryPromise`.
...
const result = await Result.tryPromise({
  ...
  catch: (cause) => new ApiError({ cause, message: "forecast API call failed" }),
});
```
...
- A tool's `execute` return value, a channel response, and anything the model or a serializer consumes must be plain serializable data — never `Ok`, `Err`, `TaggedError`, `Error`, `Response`, or other class instances.
- Unwrap the Result at the boundary and return a plain object (the model sees this value).
...
```typescript
// inside a tool execute(): convert the internal Result to a plain result the model sees
...
- For agent behavior, assert the boundary outcome in an eval — e.g. `t.calledTool("get_weather", { isError: true })` or `t.check(t.reply, includes("could not"))` — since evals see the plain tool output, not the Result.

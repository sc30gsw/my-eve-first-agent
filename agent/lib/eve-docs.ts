...
export async function listEveDocs() {
  const roots = DOC_ROOTS.map((root) => resolve(process.cwd(), root));
  const perRoot = await Promise.all(roots.map((root) => walk(root)));
...
export async function readEveDoc(path: string) {
  ...
  const allowed = DOC_ROOTS.map((root) => resolve(process.cwd(), root));
  const within = allowed.some(
